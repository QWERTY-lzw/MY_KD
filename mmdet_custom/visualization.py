...
def draw_points(ax, points, color='r', alpha=0.8, r=2):
    """Draw bounding points on the axes.
    ...
    if not isinstance(color, (list, tuple)):
        color = [color] * points.shape[0]
    assert len(color) == points.shape[0]
    for p, c in zip(points, color):
        p_int = p.astype(np.int32)
        ax.add_patch(plt.Circle((p_int[0], p_int[1]), r, color=c, alpha=alpha))
    return ax
...
def imshow_det_bboxes(img,
    ...
    """
    if isinstance(bboxes, DC):
        bboxes = bboxes.data.numpy()
    if isinstance(segms, DC):
        segms = segms.data.numpy()
    if isinstance(labels, DC):
        labels = labels.data.numpy()
    if isinstance(points, DC):
        points = points.data.numpy()
    if isinstance(point_labels, DC):
        point_labels = point_labels.data.numpy()
    ...
    max_point_label = int(max(point_labels) if point_labels is not None and len(point_labels) > 0 else 0)
    max_label = max(max_label, max_point_label)
    ...
    if points is not None:
        positions = points[:, :2].astype(np.int32) 
        # positions[:, 1:] += thickness*(thickness+1)
        positions[:, 1:] += thickness+1
        num_points= points.shape[0]
        text_colors = [text_palette[label] for label in point_labels[:num_points]]
        colors = [bbox_palette[label] for label in point_labels[:num_points]]
        draw_points(ax, points=points, color=colors, r=thickness)
        horizontal_alignment = 'center'
        scores = points[:, 2] if points.shape[1] == 3 else None
