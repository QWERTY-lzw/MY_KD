...
class FeatureLoss(nn.Module):
    ...
    def forward(self,
        ...
        Mask_Gaus = torch.zeros_like(S_attention_t)
        wmin,wmax,hmin,hmax = [],[],[],[]
        for i in range(N):
            ...
            mid_gau_h = torch.floor((hmin[i].view(1,-1) + hmax[i].view(1,-1) +1)/2)
            mid_gau_w = torch.floor((wmin[i].view(1,-1) + wmax[i].view(1,-1) +1)/2)
            gau_h = hmax[i].view(1,-1) - hmin[i].view(1,-1) + 1
            gau_w = wmax[i].view(1,-1) - wmin[i].view(1,-1) + 1
            ...
            for j in range(len(gt_bboxes[i])):
                ...
                sigma_w = 1
                sigma_h = 1
                ww = torch.arange(wmin[i][j],wmax[i][j]+1).to(Mask_fg.device) - mid_gau_w[0][j]
                hh = torch.arange(hmin[i][j],hmax[i][j]+1).to(Mask_fg.device) - mid_gau_h[0][j]
                ww = ww / gau_w[0][j]
                hh = hh / gau_h[0][j]
                hh = hh.reshape(-1,1)
                ww = ww.reshape(1,-1)
                area_Gaus = torch.exp(torch.mm(hh**2, ww**2))


                
                h_t, w_t =  Mask_Gaus[i][hmin[i][j]:hmax[i][j]+1, wmin[i][j]:wmax[i][j]+1].shape

                Mask_Gaus[i][hmin[i][j]:hmax[i][j]+1, wmin[i][j]:wmax[i][j]+1] = \
                torch.maximum(Mask_Gaus[i][hmin[i][j]:hmax[i][j]+1, wmin[i][j]:wmax[i][j]+1], area_Gaus[0:h_t, 0:w_t])
            Mask_bg[i] = torch.where(Mask_fg[i]>0, 0, 1)
            if torch.sum(Mask_bg[i]):
                Mask_bg[i] /= torch.sum(Mask_bg[i])

        fg_loss, bg_loss = self.get_fea_loss(preds_S, preds_T, Mask_fg, Mask_bg, Mask_Gaus, C_attention_s, C_attention_t, S_attention_s, S_attention_t)
    ...
    def get_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, Mask_Gaus, C_s, C_t, S_s, S_t):
        # print("------------Mask_fg.shape = ", Mask_fg.shape)
        ...
        Mask_Gaus = Mask_Gaus.unsqueeze(dim=1)
        # print("------------Mask_fg.shape = ", Mask_fg.shape)
        ...
        # print("------------fea_t.shape = ", fea_t.shape)
        # print("------------Mask_Gaus.shape = ", Mask_Gaus.shape)
        fg_fea_t = torch.mul(fg_fea_t, torch.sqrt(Mask_Gaus))

        fea_s = torch.mul(preds_S, torch.sqrt(S_t))
        fea_s = torch.mul(fea_s, torch.sqrt(C_t))
        fg_fea_s = torch.mul(fea_s, torch.sqrt(Mask_fg))
        bg_fea_s = torch.mul(fea_s, torch.sqrt(Mask_bg))
        fg_fea_s = torch.mul(fg_fea_s, torch.sqrt(Mask_Gaus))
