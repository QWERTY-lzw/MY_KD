...
train_pipeline = [
    ...
    dict(type='LoadKeypointsAnnotations', with_bbox=True, with_keypoint=True),
    dict(type='KResize', img_scale=img_scale, keep_ratio=False),
    dict(type='PhotoMetricDistortion'),
    dict(type='Corrupt', corruption='gaussian_noise'),
    dict(type='Corrupt', corruption='jpeg_compression'),
    dict(type='KRandomFlip', flip_ratio=0.),
    dict(type='Normalize', **img_norm_cfg),
    dict(type='Pad', size_divisor=32),
    dict(type='KeypointsToHeatmaps', sigma=3),
    dict(type='KeypointsFormatBundle'),
    dict(
        type='Collect',
