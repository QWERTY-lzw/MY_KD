...
valid_annotation_indices = [0, 1, 3, 4, 6, 7, 9, 10, 12, 13]
def createObjectPascalVocTree(xmin, ymin, xmax, ymax, image_width, image_height, landmarks=None):
    ...
    if landmarks:
        ld = ET.SubElement(obj, 'landmarks')
        for i,j in enumerate(valid_annotation_indices):
            ET.SubElement(ld, f'p{i}').text = str(landmarks[j])
        ...
        while line:
            ...
            while True:
                ...
                if not line or line.startswith('#'):
                    break
                i = i + 1
                infos = [float(k) for k in line.split()]
                x1, y1, w, h = infos[:4]
                assert len(infos) > 4, f'{len(infos)}'
                if len(infos) > 4:
                    assert len(infos) == 4+3*5+1
                    landmarks = infos[4:-1]
                else:
                    landmarks = None
                x2 = x1 + w
                ...
                ann.getroot().append(createObjectPascalVocTree(x1, y1, x2, y2, width, height, landmarks=landmarks).getroot())
            ...
            if ann.find("object") == None:
                ...
                with open('data/WIDERFace/train_key.txt',"a") as sf:
                    sf.write('{}\n'.format(basename[:-4]))
            if foundValidBoxes:
                xml_counter += 1
