...
def draw_keypoints(ax, keypoints, color='r', alpha=0.8, r=2):
    """Draw bounding keypoints on the axes.

    Args:
        ax (matplotlib.Axes): The input axes.
        keypoints (ndarray): The input bounding boxes with the shape
            of (n, 5, 2).
        color (list[tuple] | matplotlib.color): the colors for each
            bounding boxes.
        alpha (float): Transparency of bounding boxes. Default: 0.8.
    ...
    for points in keypoints:
        for p in points:
            ...
            ax.add_patch(plt.Circle((p_int[0], p_int[1]), r, color=color, alpha=alpha))
    return ax

def draw_fake_keypoints(ax, bboxes, color='r', alpha=0.8, thickness=2):
    """Draw bounding keypoints on the axes.
    ...
    for i, bbox in enumerate(bboxes):
        bbox_int = bbox.astype(np.int32)
        c_x, c_y = (bbox_int[0]+bbox_int[2])/2, (bbox_int[1]+bbox_int[3])/2
        ax.add_patch(plt.Circle((c_x, c_y), thickness, color=color[i], alpha=alpha))
...
def imshow_det_bboxes(img,
    ...
    if keypoints is not None:
        draw_keypoints(ax, keypoints=keypoints)
    else:
        colors = [bbox_palette[label] for label in labels[:num_bboxes]]
        # draw_fake_keypoints(ax, bboxes, colors, alpha=1, thickness=thickness+2)
        draw_fake_keypoints(ax, bboxes, alpha=1, thickness=thickness+2)
