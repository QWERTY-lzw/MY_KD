...
from utils import get_broadcast_cand, dict_to_tuple, tuple_to_dict, get_model_map
from trainer import parse_args, get_model, get_cfg
...
from mmdet_custom.models.utils.get_model_complexity_info import get_model_complexity_info as my_get_model_complexity_info
# from mmdet_custom.models.utils import USConv2d, USBatchNorm2d
# from mmdet.models.backbones.usconv import USConv2d, USBatchNorm2d
from mmdet_custom.models.utils.usconv import USConv2d, USBatchNorm2d, count_usconvNd_flops, count_usconvNd_params, count_usbn_flops, count_usbn_params
import numpy as np
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args):
        ...
        loaded_checkpoint = args.checkpoint.split("/")[1].split('.')[0]
        # loaded_checkpoint = 'test'
        times = time.strftime('%Y%m%d_%H%M%S', time.localtime())
        self.checkpoint_name = os.path.join(self.log_dir,
             '{}_info_f{}_p{}_{}.pth.tar'.format(loaded_checkpoint, self.flops_limit, self.params_limit, times))
        # default 1600*2*4
        self.logfile = os.path.join(self.log_dir, '{}_ea_f{}_p{}_{}.log'.format(loaded_checkpoint, self.flops_limit, self.params_limit,times))
    ...
    def idx_to_arch(self, cand): # cand: idx->factor
        ...
        for i in range(len(cand['widen_factor_backbone_idx'])):  # !!!here 修改cand传递形式，可能是因为broad cast无法传递0。33
            widen_factor_backbone.append(self.backbone_widen_factor_range[cand['widen_factor_backbone_idx'][i]])
        deepen_factor_backbone = []
        for i in range(len(cand['deepen_factor_backbone_idx'])):
            deepen_factor_backbone.append(self.backbone_deepen_factor_range[cand['deepen_factor_backbone_idx'][i]])
        widen_factor_neck = []
        for i in range(len(cand['widen_factor_neck_idx'])):
            widen_factor_neck.append(self.neck_widen_factor_range[cand['widen_factor_neck_idx'][i]])
        widen_factor_neck_out = self.head_widen_factor_range[cand['widen_factor_neck_out_idx']]
    ...
    def get_param(self, cand):
        ...
        # cfg = Config.fromfile('configs/search/yolox_s_8x8_300e_ship8.py')
        # cfg = Config.fromfile('configs/search/yolox_s_300e_ship8_search.py')
        cfg = self.raw_cfg.copy()

        if args.cfg_options is not None:
            cfg.merge_from_dict(args.cfg_options)

        # cfg.model.backbone.deepen_factor_backbone = arch['deepen_factor_backbone']
        ...
        head_channels = int(256*default_widen_factor*arch['widen_factor_neck_out'])
        ...
        flops, params = my_get_model_complexity_info(self.flops_model, self.input_shape, custom_ops=custom_ops)
        print("thop", round(flops / 10. ** 9, 2), round(params / 10 ** 6, 2))
    ...
    def get_random(self, num): # num=population_num
        ...
        cand_iter = self.stack_random_cand(
            lambda: {
                'widen_factor_backbone_idx': tuple([np.random.randint(0, len(self.backbone_widen_factor_range)) for i in range(5)]),
                'deepen_factor_backbone_idx': tuple([np.random.randint(0, len(self.backbone_deepen_factor_range)) for i in range(4)]),
                'widen_factor_neck_idx': tuple([np.random.randint(0, len(self.neck_widen_factor_range)) for i in range(8)]),
                'widen_factor_neck_out_idx': np.random.randint(0, len(self.head_widen_factor_range)),
            })
        ...
        def random_func(): # 以概率prob随机更改每个stage的channel和depth
            ...
            for i in range(5): # 变结构在层数里进行变异 # todo 怎么改
                if np.random.random_sample() < m_prob:
                    cand[i] = np.random.randint(0, len(self.backbone_widen_factor_range))
            for i in range(4):
                if np.random.random_sample() < m_prob:
                    cand[i + 5] = np.random.randint(0, len(self.backbone_deepen_factor_range))
            for i in range(8):
                if np.random.random_sample() < m_prob:
                    cand[i + 9] = np.random.randint(0, len(self.neck_widen_factor_range))
            if np.random.random_sample() < m_prob:
                cand[17] = np.random.randint(0, len(self.head_widen_factor_range))
            return tuple(cand)
